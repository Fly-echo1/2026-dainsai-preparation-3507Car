...
/* ===== 可调参数 ===== */
#define BASE_SPEED 100  /* 直线基础速度 (0~999 PWM)               */
#define STEER_LIMIT 350 /* 转向修正量限幅                         */

#define ANGLE_KP 6.0f /* 角度 PID 比例系数                      */
#define ANGLE_KI 0.3f /* 角度 PID 积分系数                      */
#define ANGLE_KD 0.5f /* 角度 PID 微分系数                      */

#define STATION_DEBOUNCE 20 /* 黑线去抖次数 (×10ms = 200ms)          */
#define LOOP_DELAY_MS 10    /* 控制循环周期 (ms)                      */
#define TASK_TIMEOUT_S 30   /* 超时时间 (秒)                          */
...
static volatile uint8_t testflag = 1; /* 1=纯PID调试(不停), 0=正常模式 */
...
void Task1_Run(void) {
  ...
  OLED_Clear();
  OLED_ShowString(0, 3, (uint8_t *)"GO!", 16);
  mspm0_delay_ms(300);
  ...
  while (1) {
    ...
    if (!testflag && (tick_ms - start_tick) / 1000 >= TASK_TIMEOUT_S) {
      Motor_Stop();
      OLED_Clear();
      OLED_ShowString(0, 3, (uint8_t *)"Timeout!", 12);
      mspm0_delay_ms(2000);
      task_flag = 0;
      return;
    }
